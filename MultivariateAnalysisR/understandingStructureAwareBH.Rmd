---
title: "trying to understand structure aware BH"
...
```{r}
dummyPVals <- c(0.01, 0.01,  0.001,  0.01,  0.01,      0.5 , 0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5  ) # just dummy p values to try to understand the method
n = length(dummyPVals) # here important  we need to 

roiLabels <- rep(0,n)    # dummy labels for p values we need to keep this vector of the same length as the number of p values 
...
BH_Res[BH_method(dummyPVals, alpha)] = 1
Storey_Res[Storey_method(dummyPVals, alpha, thr)] = 1
qhat = Solve_q_block(dummyPVals,tau,eps,roiLabels,ADMM_params)
SABHA_Res[SABHA_method(dummyPVals,qhat,alpha,tau)] = 1


# below we have the nummber 
...
dummyPVals <- c(0.01, 0.01,  0.001,  0.01,  0.01,      0.5 , 0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5  ) # just dummy p values to try to understand the method
n = length(dummyPVals)
roiLabels <- rep(0,n)    # dummy labels for p values
...
set up the three methods - this is from original paper so we have a way to compare the power of those statitical tests
