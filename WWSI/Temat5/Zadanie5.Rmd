...
for(i in seq(1:nbreaks)){
  ...
  d.test <- d[test_indices,1:15]  #without class
  d.test.class<- d[test_indices,15]    #only class
  d.train <- d[-test_indices, 1:15]
  d.train.class <- d[-test_indices, 15]

  #testujemy na zbiorze testowym i treningowym nasze drzewo decyzyjne

model<-rpart(cl~Age +Sex +Cp+Trestbs+ Chol+FBS+Restecg+Restecg+Thalach+Exang+Exang+Oldpeak+Slope+Slope+Slope+Ca+Thal , data = d.train, method = "class",control=rpart.control(minsplit=minSpl))
...
readARFF("Zadanie/data sets/3_cleveland_names.txt") #nowe dane sa w formacie ARFF

# najpierw definiujemy model knn i oceniamy jego błąd
  knn.result <- knn(d.train, d.test, d.train.class, k = 1)
  t<-table(d.test.class,knn.result)
  err_rateKnn<-(nrow(d.test)-sum(diag(t)))/nrow(d.test)
  print(paste ("Knn blad ", toString(err_rateKnn) ))
  
# nastepnie definiujemy drzewo decyzyjne i określamy błąd związany z nim
  model<-rpart(cl~Age +Sex +Cp+Trestbs+ Chol+FBS+Restecg+Restecg+Thalach+Exang+Exang+Oldpeak+Slope+Slope+Slope+Ca+Thal , data = d.train, method = "class",control=rpart.control(minsplit=minSpl))
  
  pred <- predict(model, d.test,type='class') 
  error_rateTree<-mean(pred != d.test.class) 
